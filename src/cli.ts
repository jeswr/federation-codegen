...
import { emitLockfile, type GenerateInput, generateModel } from "./generate.js";
...
function parseArgs(argv: string[]): Args {
  const args: Args = { updateLock: false, allowUnadmitted: false };
...
function run(argv: string[]): void {
  ...
  if (command !== "gen") fail(`unknown command "${command ?? ""}" (expected: gen)`);
  ...
  const config = JSON.parse(readFileSync(args.config, "utf8")) as CodegenConfig;

  const input: GenerateInput = { ontologyTtl, shapesTtl, config };
  if (args.allowUnadmitted) input.allowUnadmitted = true;
  ...
  let existingLock: string | undefined;
  try {
  ...
  if (existingLock !== undefined && existingLock !== freshLock && !args.updateLock) {
    fail(
      `input hashes differ from ${lockPath} — a fetched input changed. ` +
  ...
  process.stdout.write(
    `federation-codegen: generated ${Object.keys(result.artifacts).length} artifact(s) into ${args.out}\n` +
