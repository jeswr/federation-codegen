 ...
 */
import { isCompositeEntity, validateManifest, } from "@jeswr/model-runtime";
 ...
 */
export function deriveNodeViews(manifest) {
    const composites = manifest.entities.filter(isCompositeEntity);
    if (composites.length === 0)
        return undefined;
    const mainNames = new Set(manifest.entities.map((e) => e.name));
    ...
    for (const composite of composites) {
        ...
        for (const node of composite.nodes) {
            ...
            if (mainNames.has(viewName)) {
                throw new Error(`node view "${viewName}" (composite "${composite.name}", node "${node.name}") collides with a manifest entity name — rename the entity or the node`);
            }
            if (viewNames.has(viewName)) {
                throw new Error(`node view "${viewName}" (composite "${composite.name}", node "${node.name}") collides with another node view — rename the composite or the node`);
            }
...
//# sourceMappingURL=nodeviews.js.map