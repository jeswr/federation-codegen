 ...
 *   4. A `sh:in` enumeration, if present, is a non-empty closed list.
 */
...
export class AdmissionError extends Error {
    ...
    constructor(violations) {
        ...
        super(`ontology admission failed (${errors.length} error(s)):\n` +
            errors.map((v) => `  [${v.code}] ${v.subject}: ${v.message}`).join("\n"));
        this.violations = violations;
    }
}
function inNamespace(iri, namespace) {
 ...
 */
export function admit(ontology, shapes, namespace) {
    const violations = [];
    const add = (code, severity, subject, message) => violations.push({ code, severity, subject, message });
    // 1. Labels + definitions on every sector class + property.
    for (const c of ontology.classes) {
        if (!inNamespace(c.iri, namespace))
            continue;
    ...
    for (const p of ontology.properties) {
        if (!inNamespace(p.iri, namespace))
            continue;
        ...
        if (!(p.domain && p.range) && !p.unconstrainedWaiver) {
            add("missing-domain-range", "error", p.iri, "property lacks rdfs:domain + rdfs:range and has no fedgen:deliberatelyUnconstrained waiver");
        }
    }
    ...
    for (const c of ontology.classes) {
        ...
        if (!shape) {
            add("missing-shape", "error", c.iri, "instance-data class has no SHACL NodeShape (sh:targetClass)");
            continue;
        }
        for (const prop of shape.properties) {
            ...
            if (prop.minCount === undefined && prop.maxCount === undefined) {
                add("missing-cardinality", "warning", prop.pathIri, `property in ${shape.iri} states neither sh:minCount nor sh:maxCount (unbounded set — advisory)`);
            }
        }
    }
...
export function assertAdmitted(ontology, shapes, namespace) {
    const report = admit(ontology, shapes, namespace);
    if (!report.ok)
        throw new AdmissionError(report.violations);
    return report;
}
