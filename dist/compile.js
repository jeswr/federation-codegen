...
function compileField(constraint, fieldConfig, provenance) {
    ...
    if (constraint.kind === "literal" && constraint.datatype)
        field.datatype = constraint.datatype;
    if (constraint.minCount !== undefined)
        field.minCount = constraint.minCount;
    if (constraint.maxCount !== undefined)
        field.maxCount = constraint.maxCount;
    ...
    if (constraint.in !== undefined && constraint.in.length > 0)
        field.in = constraint.in;
    const guards = {};
    ...
    if (fieldConfig) {
        const record = (key) => provenance.configGuards.push(key);
...
function compileEntity(shape, config, provenance) {
    ...
    for (const configuredName of Object.keys(entityConfig.fields ?? {})) {
        if (!shapeFieldNames.has(configuredName)) {
            throw new Error(`codegen config for ${shape.targetClass} configures unknown field "${configuredName}"`);
        }
    }
    return {
